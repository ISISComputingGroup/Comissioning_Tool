...
class MotorSettings(ttk.Frame):
    ...
    def __init__(self, axis, parent, func_change_axis, available_axes):
        ...
        self.calc_steps.trace(
            "w", lambda *args: self.do_calculation(mot_steps=self.calc_steps.get())
        )
    ...
    def do_calculation(self, **vals):
        ...
        if "enc_cnts" in vals:
            stps_per_cnt = self.axis.microstep.get() / (
                self.axis.motor_res.get() * self.axis.enc_res.get()
            )
            self.calc_steps.set(vals["enc_cnts"] * stps_per_cnt)
        elif "mot_steps" in vals:
            self.calc_pos.set(
                vals["mot_steps"] * self.axis.motor_res.get() / self.axis.microstep.get()
            )
        elif "pos" in vals:
    ...
    def _set_soft_limits(self):
        ...
        self.limits_grid.set(
            self.SOFT_LABEL, column=0, value=lim_repr(self.axis.get_soft_limit(False))
        )
        self.limits_grid.set(
            self.SOFT_LABEL, column=1, value=lim_repr(self.axis.get_soft_limit(True))
        )
    ...
    def create_widgets(self, available_axes):
        ...
        ttk.Label(self, text="Axis under test: ", font="Times 20").grid(
            column=0, row=0, columnspan=2, pady="10", sticky=E
        )
        axes = ttk.OptionMenu(
            self,
            self.chosen_axis,
        ...
        ttk.Button(self, text="Jog Forward", command=lambda: self.jog_axis(True)).grid(
            column=2, row=1
        )
        ttk.Button(self, text="Jog Backward", command=lambda: self.jog_axis(False)).grid(
            column=3, row=1
        )
        ...
        self.set_lims = ttk.Button(
            self, text="Set Limits", state=DISABLED, command=self._set_soft_limits
        )
        ...
        ttk.Label(res, text="Encoder Resolution (counts/" + chr(181) + "m)").grid(
